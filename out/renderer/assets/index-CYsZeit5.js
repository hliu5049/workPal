...
var [createScrollAreaContext] = createContextScope$3(SCROLL_AREA_NAME);
...
const createLucideIcon = (iconName, iconNode) => {
  const Component = reactExports.forwardRef(
    ({ color = "currentColor", size: size2 = 24, strokeWidth = 2, absoluteStrokeWidth, className = "", children, ...rest }, ref) => {
  ...
  (props, forwardedRef) => {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsx(
      AvatarProvider,
      {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsx(Primitive$5.span, { ...avatarProps, ref: forwardedRef })
      }
    );
  }
);
...
var AvatarImage$1 = reactExports.forwardRef(
  (props, forwardedRef) => {
    ...
    return imageLoadingStatus === "loaded" ? /* @__PURE__ */ jsxRuntimeExports.jsx(Primitive$5.img, { ...imageProps, ref: forwardedRef, src }) : null;
  }
);
...
var AvatarFallback$1 = reactExports.forwardRef(
  (props, forwardedRef) => {
    ...
    return canRender && context.imageLoadingStatus !== "loaded" ? /* @__PURE__ */ jsxRuntimeExports.jsx(Primitive$5.span, { ...fallbackProps, ref: forwardedRef }) : null;
  }
);
...
function Nav({ links, isCollapsed }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsx("nav", { className: "grid gap-1 px-2 group-[[data-collapsed=true]]:justify-center group-[[data-collapsed=true]]:px-2", children: links.map((link, index2) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          type: "button",
          ...
          className: cn(
            ...
            link.variant === "default" && "bg-primary text-primary-foreground hover:bg-primary/90 hover:text-primary-foreground",
            isCollapsed && "justify-center px-2"
          ),
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(link.icon, { className: "h-4 w-4" }),
            !isCollapsed && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: link.title }),
            !isCollapsed && link.label && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "ml-auto text-muted-foreground", children: link.label })
          ]
        },
        index2
      )) })
...
function stripThinking(text) {
  return text.replace(/<think>[\s\S]*?<\/think>/gi, "").replace(/<analysis>[\s\S]*?<\/analysis>/gi, "").trim();
}
function stripThinkingForDisplay(text) {
  const withoutTags = stripThinking(text).replace(/```(?:thinking|analysis)\s*[\s\S]*?\s*```/gi, "").replace(/^\s*(thinking|analysis)\s*:[\s\S]*$/gim, "");
...
function calcTypewriterStep(totalChars) {
  if (totalChars >= 4e3) return 10;
...
function tryExtractWindowsPath(text) {
...
function inferTitle(messages) {
  const firstUser = messages.find((m2) => m2.role === "user" && m2.content.trim());
  if (!firstUser) return "New chat";
  const oneLine = firstUser.content.trim().replace(/\s+/g, " ");
  return oneLine.length > 40 ? `${oneLine.slice(0, 40)}…` : oneLine;
}
function shouldArchive(session) {
  const meaningful = session.messages.some((m2) => m2.role === "user" && m2.content.trim());
  return meaningful;
}
function Layout() {
  ...
  const [viewingSessionId, setViewingSessionId] = reactExports.useState(null);
  const [inputValue, setInputValue] = reactExports.useState("");
  const requestIdRef = reactExports.useRef(0);
  const [isGenerating, setIsGenerating] = reactExports.useState(false);
  const [typingState, setTypingState] = reactExports.useState(null);
  const typingTimerRef = reactExports.useRef(null);
  const chatScrollAreaRef = reactExports.useRef(null);
  const chatBottomRef = reactExports.useRef(null);
  const [isChatNearBottom, setIsChatNearBottom] = reactExports.useState(true);
  const [llmConfig, setLlmConfig] = reactExports.useState(null);
  const [userAvatarSrc, setUserAvatarSrc] = reactExports.useState("");
  const [aiAvatarSrc, setAiAvatarSrc] = reactExports.useState("");
  const [chiikawaStyle, setChiikawaStyle] = reactExports.useState(true);
  const [storeInfo, setStoreInfo] = reactExports.useState(null);
  const userAvatarFileRef = reactExports.useRef(null);
  const aiAvatarFileRef = reactExports.useRef(null);
  const [llmBaseUrl, setLlmBaseUrl] = reactExports.useState("");
  const [llmModel, setLlmModel] = reactExports.useState("");
  const [llmTemperature, setLlmTemperature] = reactExports.useState("0.7");
  const [llmMaxTokens, setLlmMaxTokens] = reactExports.useState("1024");
  const [llmApiKey, setLlmApiKey] = reactExports.useState("");
  const [llmClearApiKey, setLlmClearApiKey] = reactExports.useState(false);
  const [llmStatus, setLlmStatus] = reactExports.useState(null);
  const [memoryContext, setMemoryContext] = reactExports.useState("");
  const [memoryPaths, setMemoryPaths] = reactExports.useState(null);
  const [fsRoots, setFsRoots] = reactExports.useState({});
  const [lastFsPlanIds, setLastFsPlanIds] = reactExports.useState({});
  const fsPlansRef = reactExports.useRef(
    /* @__PURE__ */ new Map()
  );
  const memoryWrittenRef = reactExports.useRef(/* @__PURE__ */ new Set());
  const hasLoadedPersistentStateRef = reactExports.useRef(false);
  const historySorted = reactExports.useMemo(() => {
    return [...history].sort((a, b) => b.updatedAt - a.updatedAt);
  }, [history]);
  const viewedSession = reactExports.useMemo(() => {
  ...
  const viewedSessionId = reactExports.useMemo(() => {
  ...
  reactExports.useEffect(() => {
    void (async () => {
      try {
        ...
        const chatState = chatStateRaw ?? fallbackChat;
        ...
        const avatars = avatarRaw ?? fallbackAvatars;
        if (typeof avatars?.user === "string") setUserAvatarSrc(avatars.user);
        if (typeof avatars?.ai === "string") setAiAvatarSrc(avatars.ai);
        const ui2 = uiRaw ?? fallbackUi;
        if (typeof ui2?.chiikawaStyle === "boolean") setChiikawaStyle(ui2.chiikawaStyle);
        const roots = rootsRaw ?? fallbackRoots;
  ...
  const normalizeMemoryLine = (text) => {
    return text.replace(/\s+/g, " ").replace(/\r?\n/g, " ").trim();
  };
  const shouldConsiderAutoMemory = (userText, assistantText) => {
    return /记住|偏好|喜欢|讨厌|偏爱|以后|总是|永远|不要|别再|请用|请不要|我是|我叫|叫我/.test(
      userText + "\n" + assistantText
    );
  };
  const autoWriteMemoryFromTurn = async (userText, assistantText) => {
    ...
    const input = [
      "【已加载记忆】",
      ctxSnapshot.length > 4e3 ? `${ctxSnapshot.slice(0, 4e3)}
…（已截断）` : ctxSnapshot || "（空）",
      "",
      "【本轮对话】",
      `User: ${userText}`,
  ...
  const openSessionFromHistory = (sessionId) => {
    abortRunning();
  ...
  const saveLlmConfig = async () => {
    try {
      ...
      const temperature = Number.isFinite(Number(llmTemperature)) ? Number(llmTemperature) : void 0;
      const maxTokens = Number.isFinite(Number(llmMaxTokens)) ? Number(llmMaxTokens) : void 0;
  ...
  const summarizeEntries = (entries) => {
    ...
    for (const e of entries) {
      ...
      const m2 = e.path.toLowerCase().match(/\.([a-z0-9]+)$/);
      const ext = m2?.[1] ? `.${m2[1]}` : "(noext)";
      extCount.set(ext, (extCount.get(ext) ?? 0) + 1);
    }
    const topExt = [...extCount.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10).map(([k2, v2]) => `${k2}:${v2}`).join("  ");
    return { fileCount, dirCount, topExt };
  };
  const buildFsPrompt = (args) => {
    return [
  ...
  const replyToChat = (content) => {
    appendMessageToSession(viewingSessionId, { role: "ai", content, createdAt: Date.now() });
  };
  const scanRootToChat = async (root) => {
    try {
      const list = await window.api.fs.list({ root, maxEntries: 5e3 });
  ...
  const listRootToChat = async (root) => {
    try {
      const list = await window.api.fs.list({ root, maxEntries: 5e3 });
      const { fileCount, dirCount, topExt } = summarizeEntries(list);
      const dirs = list.filter((e) => e.type === "dir").map((e) => e.path).sort((a, b) => a.localeCompare(b));
      const files = list.filter((e) => e.type === "file").map((e) => e.path).sort((a, b) => a.localeCompare(b));
      const dirLines = dirs.slice(0, 30).map((p2) => `- ${p2}/`);
      const fileLines = files.slice(0, 30).map((p2) => `- ${p2}`);
      replyToChat(
        [
          `目录：${root}`,
  ...
  const clearPlanActions = (sessionId, planId) => {
    updateSessionMessages(
      sessionId,
      (messages) => messages.map((m2) => {
        if (!m2.actions || m2.actions.length === 0) return m2;
        const has = m2.actions.some((a) => "planId" in a ? a.planId === planId : false);
        if (!has) return m2;
        return { ...m2, actions: [] };
      })
    );
  };
  const applyPlanToChat = async (sessionId, planId) => {
    ...
    try {
      ...
      const lines = res.preview.slice(0, 40).map((p2, i) => {
        const detail = p2.detail ? ` (${p2.detail})` : "";
        return `${i + 1}. ${p2.title}${detail}`;
      });
...
${errLines.join("\n")}` : ""
  ...
  const cancelPlanToChat = (sessionId, planId) => {
    clearPlanActions(sessionId, planId);
    appendMessageToSession(sessionId, {
      role: "ai",
  ...
  const handleSendMessage = () => {
    ...
    void (async () => {
      try {
        ...
        const handleRoot = async (root) => {
          setFsRoot(root);
          try {
            const list = await window.api.fs.list({ root, maxEntries: 2e3 });
            const { fileCount, dirCount, topExt } = summarizeEntries(list);
            reply(
              [
                `已设置根目录：${root}`,
        ...
        const generatePlan = async (instruction, root) => {
          const list = await window.api.fs.list({ root, maxEntries: 5e3 });
          const filesForPrompt = list.filter((e) => e.type === "file").slice(0, 300).map((f2) => `${f2.path}`).join("\n");
          const prompt = buildFsPrompt({ instruction, root, filesForPrompt });
          const res2 = await window.api.llm.chat({ messages: [{ role: "user", content: prompt }] });
          let parsed;
          try {
            parsed = JSON.parse(extractFirstJsonObject(res2.text));
          } catch (e) {
          ...
          const previewLines = check.preview.slice(0, 40).map((p2, i) => {
            const detail = p2.detail ? ` (${p2.detail})` : "";
            return `${i + 1}. ${p2.title}${detail}`;
          });
...
${errLines.join("\n")}` : "",
        ...
        if (question.startsWith("/")) {
          ...
          if (cmd === "remember") {
            ...
            const res2 = await window.api.memory.appendLongTerm({ text });
            const ctx = await window.api.memory.getContext();
            setMemoryContext(ctx.combined);
            reply(`已写入长期记忆：${res2.path}`);
            return;
          }
          if (cmd === "note") {
            const text = argStr.trim();
            ...
            const res2 = await window.api.memory.appendDaily({ text });
            const ctx = await window.api.memory.getContext();
            setMemoryContext(ctx.combined);
            reply(`已写入短期记忆：${res2.path}`);
            return;
          }
          if (cmd === "mem") {
            ...
            if (sub === "paths") {
              if (!memoryPaths) {
                const p2 = await window.api.memory.getPaths();
                setMemoryPaths(p2);
                reply([`baseDir: ${p2.baseDir}`, `longTerm: ${p2.longTerm}`, `dailyDir: ${p2.dailyDir}`].join("\n"));
                return;
              }
              reply(
                [
            ...
            if (sub === "show") {
              const ctx = await window.api.memory.getContext();
              setMemoryContext(ctx.combined);
              const text = ctx.combined.length > 4e3 ? `${ctx.combined.slice(0, 4e3)}
…（已截断）` : ctx.combined;
              reply(text);
              return;
            }
            if (sub === "search") {
              ...
              const res2 = await window.api.memory.search({ query: subArg, limit: 20 });
              if (!res2.hits.length) {
                reply("未找到匹配内容");
                return;
              }
              reply(
                res2.hits.slice(0, 20).map((h) => `${h.file}:${h.line}  ${h.text}`).join("\n")
              );
              return;
            }
          }
          ...
          if (cmd === "apply") {
            ...
            if (planId === "latest" || !planId) {
              ...
              if (!planId) {
                let best = null;
                for (const [id2, p2] of fsPlansRef.current.entries()) {
                  if (p2.sessionId !== viewedSessionId) continue;
                  if (!best || p2.createdAt > best.createdAt) best = { id: id2, createdAt: p2.createdAt };
                }
                planId = best?.id ?? "";
              }
            }
        ...
        const fsIntent = /整理|归档|批量|重命名|移动|替换|删除|按扩展名|文件夹|目录|前缀|去掉|去除|移除/.test(question) || question.startsWith("fs:");
        ...
        if (fsIntent) {
          ...
          if (prefix) {
            ...
            setLastFsPlanIds((prev) => ({ ...prev, [viewedSessionId]: planId }));
            const previewLines = check.preview.slice(0, 40).map((p2, i) => `${i + 1}. ${p2.title}`);
            reply({
              content: [
                `已生成计划（ID: ${planId}）`,
        ...
        if (/记住这个|记住一下|记下来/.test(question)) {
          const text = question.replace(/^(请|帮我)?\s*(记住这个|记住一下|记下来)[:：\s]*/g, "").trim();
          const toSave = text || question.trim();
          const res2 = await window.api.memory.appendLongTerm({ text: toSave });
          const ctx = await window.api.memory.getContext();
          setMemoryContext(ctx.combined);
          reply(`已写入长期记忆：${res2.path}`);
          return;
        ...
        const nameA = question.match(/(?:我叫|我的名字是|叫我)\s*([^\s，。,.]{1,20})/)?.[1]?.trim() ?? "";
        ...
        const prior = viewedSession.messages.filter((m2) => m2.content.trim()).map((m2) => ({
          role: m2.role === "ai" ? "assistant" : "user",
          content: m2.content
        }));
        const res = await window.api.llm.chat({
          messages: [
            ...memoryCtxForTurn.trim() ? [{ role: "system", content: memoryCtxForTurn.trim() }] : [],
            ...prior,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(TooltipProvider, { delayDuration: 0, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "h-screen w-full flex overflow-hidden", children: [
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              ...
              children: isCollapsed ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                Button,
                {
                  size: "icon",
                  variant: "ghost",
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 font-semibold", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-6 w-6 rounded bg-primary text-primary-foreground flex items-center justify-center", children: "W" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "WorkPal" })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  Button,
                  {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 overflow-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(ScrollArea, { className: "h-full", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              Nav,
              {
                isCollapsed,
            ...
            !isCollapsed && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-2 pb-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-2 pt-2 pb-1 text-xs font-medium text-muted-foreground", children: "History" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid gap-1", children: historySorted.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-2 py-2 text-xs text-muted-foreground", children: "No history yet" }) : historySorted.map((s) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "group flex items-center gap-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
                    type: "button",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "flex-1 min-w-0 bg-background", children: activeView === "settings" ? /* @__PURE__ */ jsxRuntimeExports.jsx(ScrollArea, { className: "h-full p-6", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-2xl mx-auto space-y-6 py-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-2xl font-semibold", children: "LLM 设置" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm text-muted-foreground", children: "支持 OpenAI 兼容接口（OpenAI / Ollama / LM Studio 等）。" }),
        storeInfo?.userData && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs text-muted-foreground break-all", children: [
          "数据目录：",
          storeInfo.userData
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-medium", children: "Base URL" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Input,
          {
            value: llmBaseUrl,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-medium", children: "Model" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Input,
          {
            value: llmModel,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-medium", children: "Temperature" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            Input,
            {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-medium", children: "Max Tokens" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            Input,
            {
              value: llmMaxTokens,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between gap-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-medium", children: "API Key" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: llmConfig?.apiKeyPresent ? "已设置" : "未设置" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Input,
          {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { onClick: () => void saveLlmConfig(), children: "保存" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "secondary", onClick: () => void testLlmConfig(), children: "测试连接" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "outline", onClick: () => setActiveView("chat"), children: "返回聊天" })
      ] }),
      llmStatus && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rounded-md border bg-muted/30 p-3 text-sm whitespace-pre-wrap", children: llmStatus }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Separator, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-2xl font-semibold", children: "头像设置" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm text-muted-foreground", children: "支持输入图片 URL 或上传本地图片。" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-medium", children: "你的头像" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs(Avatar, { className: "h-10 w-10", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(AvatarImage, { src: userAvatarSrc || void 0 }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(AvatarFallback, { children: "U" })
            ] }),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                ...
                onChange: (e) => {
                  ...
                  if (!f2) return;
                  void (async () => {
                    const dataUrl = await readFileAsDataUrl(f2);
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "outline", onClick: () => userAvatarFileRef.current?.click(), children: "上传" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "secondary", onClick: () => setUserAvatarSrc(""), children: "清除" })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-medium", children: "AI 头像" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs(Avatar, { className: "h-10 w-10", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(AvatarImage, { src: aiAvatarSrc || void 0 }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(AvatarFallback, { children: "AI" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
              Input,
              {
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "outline", onClick: () => aiAvatarFileRef.current?.click(), children: "上传" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "secondary", onClick: () => setAiAvatarSrc(""), children: "清除" })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Separator, {}),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-medium", children: "界面风格" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "flex items-center gap-2 text-sm text-muted-foreground", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
    ...
    ] }) }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex h-full flex-col relative", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute top-4 right-4 z-10", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Avatar, { className: "h-8 w-8", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(AvatarImage, { src: aiAvatarSrc || void 0 }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(AvatarFallback, { children: "AI" })
      ] }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(ScrollArea, { ref: chatScrollAreaRef, className: "flex-1 p-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-3xl mx-auto flex flex-col gap-4 py-8", children: [
        viewedSession.messages.map((msg, index2) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: cn("flex gap-3", msg.role === "user" ? "flex-row-reverse" : ""), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(Avatar, { className: "h-8 w-8 shrink-0", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(AvatarImage, { src: msg.role === "ai" ? aiAvatarSrc || void 0 : userAvatarSrc || void 0 }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(AvatarFallback, { children: msg.role === "ai" ? "AI" : "U" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              ...
              children: (() => {
                ...
                return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                    shownText,
                    isTyping && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "inline-block w-2 align-baseline animate-pulse", children: "▍" })
                  ] }),
                  msg.role === "ai" && msg.actions && msg.actions.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-3 flex flex-wrap gap-2", children: msg.actions.map((a, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                    Button,
                    {
                      size: "sm",
                      ...
                      children: a.label ?? (a.type === "applyPlan" ? "确认执行" : "取消")
                    },
                    `${a.type}-${a.planId}-${i}`
                  )) })
                ] });
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-4 border-t bg-background", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-3xl mx-auto relative", children: [
          getFsRoot() && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-2 flex items-center justify-between gap-2 rounded-md border bg-muted/30 px-3 py-2 text-xs", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-center mt-2", children: /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground", children: "WorkPal can make mistakes. Please verify important information." }) })
      ] })
    ] }) })
  ] }) });
}
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-screen w-full bg-background text-foreground overflow-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Layout, {}) });
}
class ErrorBoundary extends reactExports.Component {
  ...
  render() {
    if (this.state.hasError) {
      return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-4 bg-red-100 text-red-900 h-screen overflow-auto", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-xl font-bold mb-2", children: "Something went wrong" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { className: "whitespace-pre-wrap font-mono text-sm", children: this.state.error?.toString() })
      ] });
    }
...
client.createRoot(document.getElementById("root")).render(
  /* @__PURE__ */ jsxRuntimeExports.jsx(React$1.StrictMode, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(ErrorBoundary, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(App, {}) }) })
);
